...
public:
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        // LOG(INFO)<<"预积分优化前"<<prevPose_.x()<<"   "<<prevPose_.y()<<"   "<<prevPose_.z();
        ...
        gtsam::noiseModel::Gaussian::shared_ptr updatedPoseNoise = gtsam::noiseModel::Gaussian::Covariance(optimizer.marginalCovariance(X(key-1)));
        // ROS_INFO("ACCBias: %4f    %4f    %4f",ba[0], ba[1], ba[2]);
        // ROS_INFO("GYRBias: %4f    %4f    %4f",bg[0], bg[1], bg[2]);
        // LOG(INFO)<<"ACCBias:"<<ba[0]<<","<<ba[1]<<","<<ba[2]<<"gyrBias:"<<bg[0]<<","<<bg[1]<<","<<bg[2];
        // ROS_INFO("fc: %4f    %4f    %4f",optimizer.marginalCovariance(X(key-1))(0,0), optimizer.marginalCovariance(X(key-1))(0,1), optimizer.marginalCovariance(X(key-1))(1,1));
        // LOG(INFO)<<"预积分优化后"<<prevPose_.x()<<"   "<<prevPose_.y()<<"   "<<prevPose_.z();
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // odometry.header.stamp =  thisImu.header.stamp.toSec()+0.02;
        // odometry.header.stamp.fromSec(thisImu.header.stamp.toSec() - 0.1);
