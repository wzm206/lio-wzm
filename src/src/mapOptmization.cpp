...
public:
    ...
    */
    mapOptimization()
    {
        ...
        google::SetLogDestination(google::GLOG_INFO, "/home/wzm/studyros/kf_lio_sam/src/kf-LIO-SAM-DetailedNote/src/logs/info");//INFO级别的日志都存放到logs目录下且前缀为INFO_
        google::SetLogDestination(google::GLOG_WARNING, "/home/wzm/studyros/kf_lio_sam/src/kf-LIO-SAM-DetailedNote/src/logs/warn");//WARNING级别的日志都存放到logs目录下且前缀为WARNING_
        google::SetLogDestination(google::GLOG_ERROR, "/home/wzm/studyros/kf_lio_sam/src/kf-LIO-SAM-DetailedNote/src/logs/error");	//ERROR级别的日志都存放到logs目录下且前缀为ERROR_
        google::SetLogDestination(google::GLOG_FATAL, "/home/wzm/studyros/kf_lio_sam/src/kf-LIO-SAM-DetailedNote/src/logs/fatal");	//FATAL级别的日志都存放到logs目录下且前缀为FATAL_
    ...
    */
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            // 4、发布里程计轨迹
            publishFrames();
            //LOG(INFO)<<"姿态"<<transformTobeMapped[0]*57.3<<"  "<<transformTobeMapped[1]*57.3<<"  "<<transformTobeMapped[2]*57.3<<"位移"<<transformTobeMapped[3]<<"  "<<transformTobeMapped[4]<<"  "<<transformTobeMapped[5];
            // ROS_INFO("ref: %5f  %5f  %5f  ",transformTobeMapped[0],transformTobeMapped[1],transformTobeMapped[2]);
            // ROS_INFO("guji: %5f  %5f  %5f  ",roll_kf_after, pitch_kf_after, yaw_kf_after);
    ...
    */
    void gpsHandler(const nav_msgs::Odometry::ConstPtr& gpsMsg)
    {
        ...
        // ROS_INFO("have gps now !!!");
    }

    void gnssHandler(const geometry_msgs::PoseStamped::ConstPtr& gnssMsg)
    {
        // gpsQueue.push_back(*gpsMsg);
    ...
    */
    bool saveMapService(lio_sam::save_mapRequest& req, lio_sam::save_mapResponse& res)
    {
      ...
      *globalMapCloud += *globalSurfCloud;
      ...
      /*以下是自己修改的*/
    string save_directory = saveMapDirectory;

    gtsam::Values _estimates =  isamCurrentEstimate;
    std::fstream stream1(save_directory + "optimized_odom_tum.txt",
                        std::fstream::out);
    stream1.precision(15);
    cout << "isamCurrentEstimate" <<isamCurrentEstimate.size() <<"\n"<< endl;
    cout << "_estimates" <<_estimates.size()<<"\n"<< endl;
    cout << "Saving map to pcd files completed,zhunbeixunhuan\n" << endl;
    for (int i = 0; i < _estimates.size(); i++) {
        auto &pose = _estimates.at(i).cast<gtsam::Pose3>();
        gtsam::Point3 p = pose.translation();
        gtsam::Quaternion q = pose.rotation().toQuaternion();
        stream1 << keyframeTimes.at(i)  << " " << p.x() << " " << p.y() << " "
            << p.z() << " " << q.x() << " " << q.y() << " " << q.z() << " "
            << q.w() << std::endl;
    }
    cout << "Saving map to pcd files completed,diyige\n" << endl;

  
    std::fstream stream2(save_directory + "optimized_odom_kitti.txt",
                        std::fstream::out);
    stream2.precision(15);
    for (const auto &key_value : _estimates) {
        ...
        stream2 << col1.x() << " " << col2.x() << " " << col3.x() << " " << t.x()
            << " " << col1.y() << " " << col2.y() << " " << col3.y() << " "
            << t.y() << " " << col1.z() << " " << col2.z() << " " << col3.z()
            << " " << t.z() << std::endl;
    }
    cout << "Saving map to pcd files completed,dierge\n" << endl;
    ...
    void updateInitialGuess()
    {
        ...
        if (cloudInfo.odomAvailable == true)
        {
            ...
            if (lastImuPreTransAvailable == false)
            {
                ...
                //
                transformTobeMapped[3] = 0.5*transformTobeMapped[3] + 0.5*wzm_gps_x;
                transformTobeMapped[4] = 0.5*transformTobeMapped[4] + 0.5*wzm_gps_y;
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            for (int iterCount = 0; iterCount < 30; iterCount++)
            {
                ...
                {
                    // ROS_INFO("Have %d edge and %d planar features available.", laserCloudCornerLastDSNum, laserCloudSurfLastDSNum);
                    break;          
                }
            }
    ...
    */
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-5, 1e-5, 1e-6, 1e-4, 1e-4, 1e-4).finished());
    ...
    void addGPSFactor()
    {
        ...
        // while (!gpsQueue.empty())
        {
            ...
            //         continue;
                ...
                float gps_x = wzm_gps_x;
                float gps_y = wzm_gps_y;
                float gps_z = transformTobeMapped[5];
                float noise_x = 0;
                float noise_y = 0;
                float noise_z = 0;
                ...
                ROS_INFO("\033[1;32m----> GPS  factor attend\033[0m");
                ROS_INFO("gps_factor:%f,%f, map:%f,%f",gps_x, gps_y,transformTobeMapped[3],transformTobeMapped[4]);
                // aLoopIsClosed = true;
                // break;
            // }
        }
    ...
    */
    void saveKeyFramesAndFactor()
    {
        ...
        // 激光里程计因子
        // LOG(INFO)<<"设为关键帧之前：姿态"<<transformTobeMapped[0]<<"  "<<transformTobeMapped[1]<<"  "<<transformTobeMapped[2]<<"位移"<<transformTobeMapped[3]<<"  "<<transformTobeMapped[4]<<"  "<<transformTobeMapped[5];
        addOdomFactor();
        ...
        keyframeTimes.push_back(timeLaserInfoStamp.toSec());   //自己奥

        // 更新里程计轨迹
        updatePath(thisPose6D);
        // LOG(INFO)<<"设为关键帧之后 ：姿态"<<transformTobeMapped[0]<<"  "<<transformTobeMapped[1]<<"  "<<transformTobeMapped[2]<<"位移"<<transformTobeMapped[3]<<"  "<<transformTobeMapped[4]<<"  "<<transformTobeMapped[5];
    }
    ...
    */
    void publishOdometry()
    {
        ...
        const string path_save_dir = "/home/wzm/studyros/save_all_poses.txt";
        std::ofstream pose1(path_save_dir, std::ios::app);
        pose1.setf(std::ios::scientific, std::ios::floatfield);
        pose1.precision(9);
        pose1 << laserOdometryROS.header.stamp << " "
    ...
    */
    ...
    {
        // std::lock_guard<std::mutex> lock(mtx);
        // sensor_msgs::Imu thisImu = imuConverter_kf(*imu_raw);
        // imu_kf.push_back(thisImu);

        // /*下面是kf*/
        // unique_lock<mutex> lck(imuMutex);
        
        // imu.gx = imu_raw->angular_velocity.x;
        // imu.gy = imu_raw->angular_velocity.y;
        // imu.gz = imu_raw->angular_velocity.z;

        // imu.ax = imu_raw->linear_acceleration.x;
        // imu.ay = imu_raw->linear_acceleration.y;
        // imu.az = imu_raw->linear_acceleration.z;

        // imu.qw = imu_raw->orientation.w;
        // imu.qx = imu_raw->orientation.x;
        // imu.qy = imu_raw->orientation.y;
        // imu.qz = imu_raw->orientation.z;
        ...
        // if(transformTobeMapped[2] < 1.570795 || transformTobeMapped[2] > -1.570795){fangxiang = 1;}
        // if(transformTobeMapped[2] > 1.570795 || transformTobeMapped[2] < -1.570795){fangxiang = -1;}

        // Eigen::Vector3d mag = Eigen::Vector3d(fangxiang,-transformTobeMapped[2] ,0);
        // // -1 -1 135  1 1 -45
    ...
    */
    ...
    //     // std::lock_guard<std::mutex> lock(mtx);
    //     // if (cloudKeyPoses3D->points.empty())
    //     if (cloudKeyPoses3D->points.size() < 8 )
    //     {
    //         return;
    ...
    //         for(int i =0; i<imu_kf.size(); i++)
    //         {
    //             if (imu_kf[i].linear_acceleration.x>1)    imu_kf[i].linear_acceleration.x = 1;
    //             if (imu_kf[i].linear_acceleration.y>1)    imu_kf[i].linear_acceleration.y = 1;
    //             if (imu_kf[i].linear_acceleration.z>9.8+1)    imu_kf[i].linear_acceleration.z = 9.8+1;
    //             if (imu_kf[i].linear_acceleration.x< -1)    imu_kf[i].linear_acceleration.x = -1;
    //             if (imu_kf[i].linear_acceleration.y<-1)    imu_kf[i].linear_acceleration.y = -1;
    //             if (imu_kf[i].linear_acceleration.z<9.8-1)    imu_kf[i].linear_acceleration.z = 9.8-1;
                
    //             acc_x += imu_kf[i].linear_acceleration.x;
    //             acc_y += imu_kf[i].linear_acceleration.y;
    //             acc_z += imu_kf[i].linear_acceleration.z;
    //         }
    //         acc_x /= imu_kf.size();     acc_y /= imu_kf.size();     acc_z /= imu_kf.size();

    //         kf_roll = acc_x;     kf_pitch = acc_y;

    //         // ROS_INFO("aver_acc: %4f,%4f,%4f",acc_x,acc_y,acc_z);

    //         imu_kf.clear();

    //         // 提取姿态角
    //         // tf::Quaternion orientation_tf;
    //         tf::Quaternion tf_q, tf_after;
    //         // tf::quaternionMsgToTF(thisImu.orientation, orientation_tf);

    //         double roll_kf, pitch_kf, yaw_kf;
            
    //         // int numPoses = isamCurrentEstimate.size();
    //         // roll_inter = isamCurrentEstimate.at<Pose3>(numPoses-3).rotation().roll();
    //         // pitch_inter = isamCurrentEstimate.at<Pose3>(numPoses-3).rotation().pitch();
    //         // yaw_inter = isamCurrentEstimate.at<Pose3>(numPoses-3).rotation().yaw();

    //         roll_inter = transformTobeMapped[0];
    //         pitch_inter = transformTobeMapped[1];
    //         yaw_inter =  transformTobeMapped[2];

    //         // tf::Matrix3x3(orientation_tf).getRotation

    //         tf_q = tf::createQuaternionFromYaw( yaw_inter - yaw_kf );    //  theta是绕z轴旋转角,其他两个轴旋转角为0;
    //         // tf_after = tf_q*orientation_tf;
    //         // tf::Matrix3x3(tf_after).getRPY(roll_kf_after, pitch_kf_after, yaw_kf_after);
    //         // Eigen::Quaterniond rot_gravity(tf_after);

    //         tf_test = tf::createQuaternionFromRPY(roll_inter,pitch_inter,yaw_inter);
    //         tf_q = tf::createQuaternionFromYaw( (-yaw_inter) );    //  theta是绕z轴旋转角,其他两个轴旋转角为0;
    //         tf_test = tf_q*tf_test;
    //         Eigen::Quaterniond test4(tf_test);
    //         Eigen::Vector3d test2(0,0,1);
    //         test5 = test4*test2;

    //         noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-4, 1e-4, 1e-6, 1e-4, 1e-4, 1e-2).finished());
    //         gtsam::Pose3 poseFrom = pclPointTogtsamPose3(cloudKeyPoses6D->points.back());
    //         float dipingmian[6];    
    //         dipingmian[0] =0; dipingmian[1] = 0; 
    //         dipingmian[2] = transformTobeMapped[2]; dipingmian[3] = transformTobeMapped[3]; 
    //         dipingmian[4] = transformTobeMapped[4]; dipingmian[5] = transformTobeMapped[5]; 
    //         gtsam::Pose3 poseTodipingmian   = trans2gtsamPose(dipingmian);
            
    //         // 参数：前一帧id，当前帧id，前一帧与当前帧的位姿变换（作为观测值），噪声协方差
    //         // gtSAMgraph.add(BetweenFactor<Pose3>(cloudKeyPoses3D->size()-1, cloudKeyPoses3D->size(), poseFrom.between(poseTodipingmian), odometryNoise));
    //         // // 变量节点设置初始值s
    //         // initialEstimate.insert(cloudKeyPoses3D->size(), poseTo);
    ...
    //         // 添加重力因子
    //         gtsam::Vector Vector3(3);
    //         noiseModel::Diagonal::shared_ptr gravity_noise = noiseModel::Diagonal::Variances((Vector(2) << 1e-4, 1e-4).finished());
    //         gtsam::Pose3AttitudeFactor attitute_factor(cloudKeyPoses3D->size(),  Unit3(-acc_x,-acc_y,acc_z), gravity_noise,Unit3(test5[0],test5[1],test5[2]));
    //         // gtsam::Pose3AttitudeFactor attitute_factor(cloudKeyPoses3D->size(),  Unit3(0,0,1), gravity_noise,Unit3(test5[0],test5[1],test5[2]));
    //         gtSAMgraph.add(attitute_factor);
